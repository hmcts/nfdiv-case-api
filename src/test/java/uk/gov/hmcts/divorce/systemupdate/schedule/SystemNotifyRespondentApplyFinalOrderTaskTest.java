...
class SystemNotifyRespondentApplyFinalOrderTaskTest {
    ...
    void shouldTriggerNotifyRespondentEventWhenSearchReturnsNonEmptyList() {
        ...
        verify(ccdUpdateService).submitEvent(1L, SYSTEM_NOTIFY_RESPONDENT_APPLY_FINAL_ORDER, user, SERVICE_AUTHORIZATION);
    ...
    void shouldStopProcessingIfThereIsConflictDuringSubmission() {
        ...
        doThrow(new CcdConflictException("Case is modified by another transaction", mock(FeignException.class)))
            .when(ccdUpdateService).submitEvent(1L, SYSTEM_NOTIFY_RESPONDENT_APPLY_FINAL_ORDER, user, SERVICE_AUTHORIZATION);

        systemNotifyRespondentApplyFinalOrderTask.run();

        verify(ccdUpdateService)
            .submitEvent(1L, SYSTEM_NOTIFY_RESPONDENT_APPLY_FINAL_ORDER, user, SERVICE_AUTHORIZATION);
