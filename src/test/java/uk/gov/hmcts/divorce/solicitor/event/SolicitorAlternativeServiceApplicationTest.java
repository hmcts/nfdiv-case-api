...
import static uk.gov.hmcts.divorce.solicitor.event.SolicitorAlternativeServiceApplication.SOLICITOR_ALTERNATIVE_SERVICE_APPLICATION;
import static uk.gov.hmcts.divorce.testutil.ConfigTestUtil.createCaseDataConfigBuilder;
import static uk.gov.hmcts.divorce.testutil.ConfigTestUtil.getEventsFrom;
import static uk.gov.hmcts.divorce.testutil.TestConstants.TEST_CASE_ID;

@ExtendWith(MockitoExtension.class)
class SolicitorAlternativeServiceApplicationTest {

    @Mock
    private ServiceApplicationDraftSubmissionService serviceApplicationDraftSubmissionService;

    @InjectMocks
    private SolicitorAlternativeServiceApplication solicitorAlternativeServiceApplication;

    @Test
    void shouldAddConfigurationToConfigBuilder() {
        final ConfigBuilderImpl<CaseData, State, UserRole> configBuilder = createCaseDataConfigBuilder();

        solicitorAlternativeServiceApplication.configure(configBuilder);

        assertThat(getEventsFrom(configBuilder).values())
            .extracting(Event::getId)
            .contains(SOLICITOR_ALTERNATIVE_SERVICE_APPLICATION);
    }
    ...
    @Test
    void shouldSetInterimApplicationTypeToAlternativeServiceAndSubmitFromInterimOptionsOnAboutToSubmit() {
        ...
        AboutToStartOrSubmitResponse<CaseData, State> response =
            solicitorAlternativeServiceApplication.aboutToSubmit(caseDetails, caseDetails);

        assertThat(options.getInterimApplicationType()).isEqualTo(InterimApplicationType.ALTERNATIVE_SERVICE);
