...
public class ApplicationTemplateDataProvider {
    ...
    public List<Connection> deriveSoleJurisdictionList(final Application application) {
        ...
        if (null != connections) {
            ...
            if (connections.contains(APP_1_APP_2_RESIDENT)) {
                jurisdictionList.add(new Connection("the applicant and respondent are habitually resident in "
                    + "England and Wales"));
            }

            if (connections.contains(APP_1_APP_2_LAST_RESIDENT)) {
                jurisdictionList.add(new Connection("the applicant and respondent were last habitually resident "
                    + "in England and Wales and one of them still resides there"));
            }

            if (connections.contains(APP_1_RESIDENT_TWELVE_MONTHS)) {
                jurisdictionList.add(new Connection("the applicant is habitually resident in England and Wales "
                    + "and has resided there for at least a year immediately prior to the presentation of the application"));
            }

            if (connections.contains(APP_1_RESIDENT_SIX_MONTHS)) {
                jurisdictionList.add(new Connection("the applicant is domiciled and habitually resident in England "
                    + "and Wales and has resided there for at least six months immediately prior to the application"));
            }

            if (connections.contains(APP_1_APP_2_DOMICILED)) {
                jurisdictionList.add(new Connection("the applicant and respondent are both domiciled in England and Wales"));
            }

            if (connections.contains(RESIDUAL_JURISDICTION)) {
                jurisdictionList.add(new Connection("the applicant and respondent registered as civil partners "
