...
public class GenerateJudicialSeparationCORefusedForAmendmentCoverLetter {
    ...
    private Map<String, Object> templateContent(final CaseData caseData, final Long ccdCaseReference, final Applicant applicant) {
        ...
        if (caseData.getDivorceOrDissolution().isDivorce()) {
            templateContent.put(MARRIAGE_OR_CIVIL_PARTNERSHIP, WELSH.equals(applicant.getLanguagePreference())
                ? MARRIAGE_CY
                : MARRIAGE);
            templateContent.put(DIVORCE_OR_CIVIL_PARTNERSHIP, WELSH.equals(applicant.getLanguagePreference())
                ? DIVORCE_WELSH
                : DIVORCE);
        } else {
            templateContent.put(MARRIAGE_OR_CIVIL_PARTNERSHIP, WELSH.equals(applicant.getLanguagePreference())
                ? CIVIL_PARTNERSHIP_CY
                : CIVIL_PARTNERSHIP);
            templateContent.put(DIVORCE_OR_CIVIL_PARTNERSHIP, WELSH.equals(applicant.getLanguagePreference())
                ? CIVIL_PARTNERSHIP_CY
                : CIVIL_PARTNERSHIP);
