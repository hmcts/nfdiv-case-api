...
public class SystemRemindApplicant1ApplicationApprovedTask implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            final List<CaseDetails> casesInAwaitingApplicant1Response =
                ccdSearchService.searchForAllCasesWithQuery(query, user, serviceAuthorization, Applicant2Approved);
            ...
            for (final CaseDetails caseDetails : casesInAwaitingApplicant1Response) {
                try {
                    ...
                    if (!reminderDate.isAfter(LocalDate.now())
                        ...
                        notifyApplicant1(caseDetails.getId(), reminderDate, user, serviceAuthorization);
                    } else {
    ...
    private void notifyApplicant1(Long caseId, LocalDate reminderDate, User user, String serviceAuth) {
        log.info("Reminder date {} for Case id {} is on/before current date - sending reminder to Applicant 1",
            reminderDate,
            caseId
        );

        ccdUpdateService.submitEvent(caseId, SYSTEM_REMIND_APPLICANT_1_APPLICATION_REVIEWED, user, serviceAuth);
    }
}
