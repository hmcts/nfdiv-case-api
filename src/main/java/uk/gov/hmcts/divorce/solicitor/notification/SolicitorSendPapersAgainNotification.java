...
public class SolicitorSendPapersAgainNotification implements ApplicantNotification {
    ...
    public void sendToApplicant1Solicitor(final CaseData caseData, final Long caseId) {
        if (caseData.getApplicationType().isSole()) {
            ...
            String serviceMethodText = caseData.getApplication().isCourtServiceMethod()
                ? COURT_SERVICE_TEXT
                : SOLICITOR_SERVICE_TEXT;

            templateVars.put(SERVICE_METHOD, format(serviceMethodText, divorceOrDissolution));
