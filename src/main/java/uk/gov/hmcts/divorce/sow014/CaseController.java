...
public class CaseController {
    ...
    private void dispatchSubmitted(POCCaseEvent event) {
        try {
            ...
            // TODO: There is a config generator classpath bug
            //  - this exception is thrown when a callback doesn't exist for an event!
    ...
    private void saveCase(POCCaseEvent event) {
        ...
        var rowsAffected = db.update("""
                 insert into case_data (jurisdiction, case_type_id, state, data, data_classification,
                reference, security_classification, version)
                 -- TODO: separate private data model from public view
                 --  so we don't duplicate eg. notes in the json
    ...
    private POCCaseEvent dispatchAboutToSubmit(POCCaseEvent event) {
        try {
            ...
            event.getCaseDetails().put("case_data",
                mapper.readValue(mapper.writeValueAsString(cb.getData()), Map.class));
