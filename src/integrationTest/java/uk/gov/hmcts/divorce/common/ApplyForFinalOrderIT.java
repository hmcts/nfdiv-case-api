...
public class ApplyForFinalOrderIT {
    ...
    void shouldSendEmailInWelshToApplicantInAwaitingFinalOrderStateInSoleApplication() throws Exception {
        ...
        verify(notificationService)
            .sendEmail(eq(TEST_USER_EMAIL), eq(SOLE_APPLIED_FOR_FINAL_ORDER), anyMap(), eq(WELSH), anyLong());
        verifyNoMoreInteractions(notificationService);
    }
    ...
    void shouldSendApplicant2SolicitorNotificationIfJointApplicationAndApplicant2HasNotAppliedForFinalOrderYet() throws Exception {
        ...
        verify(notificationService)
            .sendEmail(eq(TEST_USER_EMAIL), eq(JOINT_ONE_APPLICANT_APPLIED_FOR_FINAL_ORDER), anyMap(), eq(ENGLISH), anyLong());
    ...
    void shouldSendApplicant1SolicitorNotificationWhenJointApplicationAndApplicant1HasAppliedForFinalOrder() throws Exception {
        ...
        verify(notificationService)
            .sendEmail(eq(TEST_SOLICITOR_EMAIL), eq(JOINT_SOLICITOR_APPLIED_FOR_CO_OR_FO_ORDER), anyMap(), eq(ENGLISH), anyLong());

        verify(notificationService)
            .sendEmail(eq("app2sol@email.com"), eq(JOINT_SOLICITOR_OTHER_PARTY_APPLIED_FOR_FINAL_ORDER), anyMap(), eq(ENGLISH), anyLong());
    ...
    void shouldSendEmailToApplicant1AndApplicant2InWelshWhenApplicant1AppliedForFOFirstInTimeJointApplication() throws Exception {
        ...
        verify(notificationService)
            .sendEmail(eq(TEST_USER_EMAIL), eq(JOINT_ONE_APPLICANT_APPLIED_FOR_FINAL_ORDER), anyMap(), eq(WELSH), anyLong());
