...
public class RespondentFinalOrderPaymentMadeIT {
    ...
    void shouldReturnErrorsIfPaymentWasUnsuccessful() throws Exception {
        ...
        final CaseDetails<CaseData, State> caseDetails = buildTestData();
        final CaseData data = caseDetails.getData();
        List<ListValue<Payment>> payments = singletonList(
            new ListValue<>("1", Payment.builder().amount(55000).status(PaymentStatus.CANCELLED).build())
        );
        data.getFinalOrder().setFinalOrderPayments(payments);

        performRespondentFinalOrderPaymentMadeRequest(data, ABOUT_TO_SUBMIT_URL)
    ...
    void shouldChangeCaseStateIfPaymentWasSuccessful() throws Exception {
        ...
        final CaseData data = caseDetails.getData();
        List<ListValue<Payment>> payments = singletonList(
            new ListValue<>("1", Payment.builder().amount(55000).status(PaymentStatus.SUCCESS).build())
        );
        data.getFinalOrder().setFinalOrderPayments(payments);
    ...
    private CaseDetails<CaseData, State> buildTestData() {
