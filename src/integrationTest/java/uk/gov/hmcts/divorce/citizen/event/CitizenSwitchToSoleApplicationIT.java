...
public class CitizenSwitchToSoleApplicationIT {
    ...
    @Test
    public void givenValidCaseDataWhenCallbackIsInvokedThenCaseIsWithdrawnAndNotificationsSent() throws Exception {
        CaseData data = validJointApplicant1CaseData();

        String actualResponse = mockMvc.perform(post(ABOUT_TO_SUBMIT_URL)
            .contentType(APPLICATION_JSON)
            .header(SERVICE_AUTHORIZATION, AUTH_HEADER_VALUE)
            .content(OBJECT_MAPPER.writeValueAsString(callbackRequest(data, SWITCH_TO_SOLE)))
