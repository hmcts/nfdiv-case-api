...
public class CitizenSubmitAosIT {
    ...
    public void givenValidCaseDataWithDisputeWhenCallbackIsInvokedThenSendEmailToApplicantAndRespondent() throws Exception {
        ...
        verify(notificationService)
            .sendEmail(eq(TEST_USER_EMAIL), eq(SOL_APPLICANT_DISPUTED_AOS_SUBMITTED), anyMap(), eq(ENGLISH));

        verify(notificationService)
            .sendEmail(eq(TEST_APPLICANT_2_USER_EMAIL), eq(SOL_RESPONDENT_DISPUTED_AOS_SUBMITTED), anyMap(), eq(ENGLISH));
