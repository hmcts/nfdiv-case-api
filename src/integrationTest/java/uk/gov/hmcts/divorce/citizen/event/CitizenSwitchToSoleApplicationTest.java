...
public class CitizenSwitchToSoleApplicationTest {
    ...
    @Test
    public void givenValidCaseDataWhenCallbackIsInvokedThenCaseIsWithdrawn() throws Exception {
        CaseData data = validJointApplicant1CaseDataMap();

        String actualResponse = mockMvc.perform(post(ABOUT_TO_SUBMIT_URL)
            .contentType(APPLICATION_JSON)
            .header(SERVICE_AUTHORIZATION, AUTH_HEADER_VALUE)
            .content(OBJECT_MAPPER.writeValueAsString(callbackRequest(data, WITHDRAW_JOINT_APPLICATION)))
