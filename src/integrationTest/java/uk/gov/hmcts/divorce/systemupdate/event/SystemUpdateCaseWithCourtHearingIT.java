...
public class SystemUpdateCaseWithCourtHearingIT {
    ...
    public void givenValidCaseDataWhenCallbackIsInvokedThenSendEmailToApplicantAndRespondent() throws Exception {
        ...
        verify(notificationService)
            .sendEmail(eq(TEST_USER_EMAIL), eq(ENTITLEMENT_GRANTED_CONDITIONAL_ORDER), anyMap(), eq(ENGLISH));

        verify(notificationService)
            .sendEmail(eq(TEST_APPLICANT_2_USER_EMAIL), eq(ENTITLEMENT_GRANTED_CONDITIONAL_ORDER), anyMap(), eq(ENGLISH));
