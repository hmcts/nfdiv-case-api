...
IDAM_CLIENT_SECRET=$(az keyvault secret show --vault-name nfdiv-aat -o tsv --query value --name idam-secret)
...
for user in $users; do
  ...
  idamToken=$(curl --insecure --fail --show-error --silent -X POST -H "Content-Type: application/x-www-form-urlencoded" --user "divorce:${IDAM_CLIENT_SECRET}" "${IDAM_API_BASE_URL}/oauth2/token?code=${code}&redirect_uri=${REDIRECT_URI}&grant_type=authorization_code" -d "" | docker run --rm --interactive stedolan/jq -r .access_token)
